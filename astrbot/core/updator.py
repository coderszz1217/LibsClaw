...
class AstrBotUpdator(RepoZipUpdator):
    """LibsClaw 更新器，继承自 RepoZipUpdator 类
    该类用于处理 LibsClaw 的更新操作
    ...
    async def update(
        ...
        # 在线更新已在此发行版中禁用
        raise RuntimeError("在线更新功能已在此发行版中禁用。")
    ...
    async def download_update_package(
        ...
        """Download an LibsClaw core update package without applying it.
        ...
        if os.environ.get("ASTRBOT_CLI") or os.environ.get("ASTRBOT_LAUNCHER"):
            raise Exception(
                "Error: You are running LibsClaw via CLI, please use `pip` or `uv tool upgrade` to update LibsClaw."
            )  # 避免版本管理混乱
        ...
        logger.info(f"准备更新至指定版本的 LibsClaw Core: {version}")
        ...
        if hosted_package_url:
            try:
                logger.info(
                    f"优先从托管存储下载 LibsClaw Core 更新包: {hosted_package_url}"
                )
                ...
                logger.warning(
                    f"从托管存储下载 LibsClaw Core 更新包失败: {exc}，"
                    "将回退到当前更新源。"
                )
    ...
    def apply_update_package(self, zip_path: str | Path) -> None:
        """Apply a previously downloaded LibsClaw core update package.
        ...
        logger.info("下载 LibsClaw Core 更新文件完成，正在执行解压...")
